...
var active, WORKER_CHILD_KILL_GRACE_MS;
var init_active_child = __esm({
  "src/engine/active-child.ts"() {
    "use strict";
    active = /* @__PURE__ */ new Set();
    WORKER_CHILD_KILL_GRACE_MS = 5e3;
  }
});
...
function resolveStepTimeoutMs(stepTimeout, workflowTimeout, engineTimeout) {
...
var import_yaml2, coerceTimeout;
var init_workflow = __esm({
  "src/engine/workflow.ts"() {
    "use strict";
    import_yaml2 = __toESM(require_dist(), 1);
    init_exec();
    coerceTimeout = (v) => typeof v === "number" && Number.isInteger(v) && v > 0 ? v : void 0;
  }
});
...
function parseLogTail(text) {
  ...
  for (let i = events.length - 1; i > lastStartIdx; i--) {
    const e = events[i];
    if (e.event === "cycle.killed" && (e.cycle_id === cycleId || e.cycle_id === void 0)) {
      killedIdx = i;
      break;
    }
  }
...
function execBashStep(repoRoot, command, env, shell = resolveShell({
  ...
  return new Promise((resolve2) => {
    ...
    child.on("error", (err) => {
      unregisterActiveChild(child.pid);
      resolve2({ status: "failed", exitCode: -1, stdout, stderr: stderr + String(err) });
    });
    child.on("close", (code) => {
      unregisterActiveChild(child.pid);
      resolve2({
...
${wr.stderr}`, MAX_STEP_END_STDERR) : truncateHeadCapped(wr.stderr, MAX_STEP_END_STDERR);
      ...
      if (r.timedOut) {
        await log2.emit("step.timeout", { cycle_id: cycleId, step: step.name, limit_ms: step.timeout_ms ?? null });
      }
      if (step.agent !== "bash") {
...
__export(run_one_exports, {
  ...
  reapAndExit: () => reapAndExit,
  runOne: () => runOne
});
function reapAndExit(sig, code, deps) {
  const n = deps.count();
  if (n === 0) {
    deps.exit(code);
    return;
...
`);
  ...
  const ci = deps.clearIntervalFn ?? ((h) => clearInterval(h));
  const poll = si(() => {
    if (!deps.anyAlive()) {
      ci(poll);
      deps.exit(code);
...
function onWorkerSignal(sig, code) {
...
async function runDoctor({ cwd: cwd2, workflow, env }) {
  const sourceEnv = env ?? process.env;
  let cfg2;
  try {
    cfg2 = await loadConfig(cwd2, sourceEnv);
  } catch (err) {
...
var ALREADY_RUNNING_CODE = "ENGINE_ALREADY_RUNNING";
var LOCK_HELD_EXIT_CODE = 75;
function alreadyRunning(pid) {
  const e = new Error(`engine already running, pid ${pid}`);
...
function acquireLock(lockPath2, deps = defaultDeps2) {
  let raw;
  try {
    raw = deps.readFileSync(lockPath2, "utf8").trim();
  } catch (e) {
    const err = e;
...
if (argv[0] === "doctor" || argv[0] === "preflight") {
  ...
  const result = await runDoctor2({ cwd: process.cwd(), workflow });
  if (result.stdout) process.stdout.write(result.stdout);
  if (result.stderr) process.stderr.write(result.stderr + String.fromCharCode(10));
  process.exit(result.exitCode);
}
...
var lockPath = join25(realpathSync(cwd), ".cycle", "engine.lock");
try {
  acquireLock(lockPath);
} catch (err) {
  console.error(err.message);
  const code = err.code === ALREADY_RUNNING_CODE ? LOCK_HELD_EXIT_CODE : 1;
  process.exit(code);
}
...
var activeCycleId;
var activeWorker;
var WORKER_KILL_GRACE_MS = 5e3;
var signalHandled = false;
function handleSupervisorSignal(sig, code) {
